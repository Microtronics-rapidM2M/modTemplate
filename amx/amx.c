...
int AMXAPI amx_GetTag(AMX *amx, int index, char **tagname, cell *tag_id)
{
  ...
  *tagname=GETENTRYNAME(hdr,tag);
  *tag_id=tag->address&TAGMASK;
...
int AMXAPI amx_Allot(AMX *amx,int cells,cell **address)
{
  ...
  if ((int)(amx->stk - amx->hea - cells*sizeof(cell)) < STKMARGIN)
...
cell* AMXAPI amx_VerifyAddress(AMX *amx, cell address, cell size)
{
  AMX_HEADER *hdr;
  unsigned char *data;
  unsigned char *xaddr;

  xaddr=(unsigned char*)amx_Address(amx,address);
  hdr=(AMX_HEADER *)amx->base;
  data=(amx->data!=NULL) ? amx->data : amx->base+(int)hdr->dat;

  /* verify that address is within amx data memory and cell aligned */
  if(((int)xaddr & (sizeof(cell)-1)) ||
     (xaddr < data) || (xaddr >= (data+amx->stp)) ||
     ((xaddr+size) < data) || ((xaddr+size) >= (data+amx->stp)) ||
     (size<0))
  {
    xaddr = NULL;
